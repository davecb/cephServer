...
func New(t trace.Trace) *S3Proto {
	...
	defer t.Begin(t)()
	var p = S3Proto{
		endpoint: "http://10.92.100.1:7480",
		verbose:  true,
...
func (p S3Proto) Get(key, bucket string) ([]byte, *s3.HeadObjectOutput, error) {
	defer p.t.Begin(p.endpoint, key, bucket)()
	...
	if err != nil {
		p.t.Printf("HeadObject err %v", err)  // log this
	} //else {
...
func (p S3Proto) Put(contents, path, bucket string) error {
	defer p.t.Begin("<contents>", path, bucket)()
...
func mustCreateService(p *S3Proto) {
	...
	if err != nil {
		...
		p.t.Printf("in mustCreateService, session.NewSession() = %v\n", sess)
	}
	p.svc = s3.New(sess, cfg)
	if p.svc == nil {
		panic(fmt.Errorf("in mustCreateService, s3.New returned a nil session"))
	}  else {
		p.t.Printf("in mustCreateService, s3.New() = %v\n", p.svc)
	}
}
